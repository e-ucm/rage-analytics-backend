...
function start() {
  ...
  mkdir -p data/{kafka,mongo,zookeeper,realtime3,elastic} >/dev/null 2>&1
  
  launch_and_wait 5 mongo elastic kzk
  wait_for_service kzk 9092 'Apache Kafka'
  wait_for_service kzk 2181 'Apache ZooKeeper'
  wait_for_service mongo 27017 'MongoDB'
  wait_for_service elastic 9200 'Elasticsearch'
  
  launch_and_wait 5 nimbus kibana
  wait_for_service nimbus 6627 'Apache Storm - Nimbus'
  wait_for_service kibana 5601 'Kibana'
  
  launch_and_wait 2 realtime
  launch_and_wait 2 supervisor ui
  docker exec -u root supervisor chmod 777 /app/output/realtime-jar-with-dependencies.jar
  docker exec -u root supervisor cp /app/output/realtime-jar-with-dependencies.jar /app/output2/realtime-jar-with-dependencies.jar
...
function report() {
  ...
  sha1sum storm-*.sh *.yml >> ${REPORT_FILE}
