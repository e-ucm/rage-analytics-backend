 ...
 *      1 - Invoke 'start' with a valid tracking code. Include <Content-Type, application/json> header if you want to send a body.
            1.1 - If you want to be an Anonymous user, just invoke start. Also, if you want to re-use an Anonymous user and create another session, you can send the body: { "anonymous" : "player_id" }.
            1.2 - If you want authorized users, after they login into A2 and obtain the user token, you have to include an authorization header with the user token that you've. <Authorization, Bearer user_token>.
 *      2 - You receive a response with the authorization token, player id, player name (as animal name), a session counter, the actor that you have to include in xAPI traces, the objectId that goes after every xAPI object identifier, and the date of the first session and current session.
                ...
                if (version) {
                    return players.findByAuthorization(body, authorization, username)
                        .then(function (player) {
                            if (player) {
                                return authTokens.start(player._id, version)
