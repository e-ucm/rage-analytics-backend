...
module.exports = (function () {
    ...
    activities.getAttempts = function (username, activityId) {
        activities.isAuthorizedFor(activityId, username, 'get', 'attempts').then(function (activity, isAuthorized) {
            if (isAuthorized) {
    ...
    activities.getUserAttempts = function (username, activityId, ofUser) {
        ...
        activities.isAuthorizedFor(activityId, username, 'get', call)
            .then(function (activity, isAuthorized) {
                if (isAuthorized) {
    ...
    activities.isAuthorizedFor = function(activityId, username, method, call) {
        ...
        activities.findOne(activityId, function (activity) {
            if (activity) {
                var typeComp = function (type) {
                    return getBestType(minType, type) === type;
                };
                if (activity.groupings.length > 0) {
                ...
                return classes.getUserType(activity.classId).then(typeComp);
            }
        });
    };
