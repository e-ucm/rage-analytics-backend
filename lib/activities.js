...
module.exports = (function () {
     ...
     */
    activities.createActivity = function (gameId, versionId, classId, username, name, rootId, parentId, offline, allowAnonymous, esClient) {
    ...
    activities.modifyActivity = function (activityId, body, add, esClient) {
        ...
        return activities.find(activities.toObjectID(activityId), true)
            .then(function (activity) {
                ...
                if (body.parentId) {
                    activities.setParent(activity._id, body.parentId, esClient);
                }
    ...
    activities.switchParent = function(child, originalParent, newParent, esClient) {
        ...
        var setRootId = function(rootActivity) {
            activities.setRoot(child, rootActivity)
                .then(function() {
                    deferred.resolve();
                })
                .fail(function(error) {
    ...
    activities.relate = function(parent, child, esClient) {
        ...
        if (!parent.rootId) {
            activities.setRoot(parent, parent, esClient)
                .then(function() {
    ...
    activities.addParentToChildMultiLevel = function(parent, child, esClient) {
        ...
        activities.getMultiLevelObject(child)
    ...
    activities.addChildToParentMultiLevel = function(parent, child, esClient) {
        ...
        activities.getMultiLevelObject(parent)
            .then(function(mlobject) {
        ...
        activities.getMultiLevelObject(parent)
            .then(function(mlobject) {
                ...
                if (!mlobject.parentId && mlobject.children.length == 0) {
                    activties.deleteMultiLevelIndex(parent)
                        .then(function() {
                            deferred.resolve();
                        })
                        .fail(function(error) {
        ...
        if (activity.rootId) {
            esClient.search({
                ...
                if (error) {
                    ...
                    if (response.hits && response.hits.hits.length) {
                        deferred.resolve(response.hits.hits[0]);
                    }else {
    ...
    activities.setRoot = function(activity, rootActivity, esClient) {
        return activities.findAndModify(activity._id, { rootId: rootActivity ? rootActivity._id : null });
    };
    ...
    activities.children = function (activityId) {
        return activities.find({parent: activities.toObjectID(activityId)}, false);
    };
