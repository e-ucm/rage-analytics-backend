...
module.exports = (function () {
     ...
     */
    activities.createActivity = function (gameId, versionId, classId, username, name, rootId) {
    ...
    activities.kibana.generateDashboard = function(activity, panels, uiStates) {
        console.log('Activities.kibana -> generateDashboard');
        return {
    ...
    activities.kibana.getKibanaBaseVisualizations = function(config, activity, createActivity) {
        var deferred = Q.defer();

        console.log('Activities.kibana.getKibanaBaseVisualizations() -> Started');

        kibana.getVisualizations('tch', activity.gameId, esClient)
            .then(function(visualizations) {
                deferred.resolve(visualizations);
            })
            .fail(function(error) {
                console.log('Activities.kibana.getKibanaBaseVisualizations() -> ERROR!');
                deferred.reject(error);
            });

        return deferred.promise;createActivity
    };

    activities.kibana.createIndex = function(config, activity, user, esClient) {
        var deferred = Q.defer();
        ...
        if(activity.rootId && activity.rootId !== ''){
            ...
            kibana.createRequiredIndexes(activity._id.toString(), activity.gameId, user, config, esClient)
                .then(function(ret) {
                    console.log('Activities.kibana.createIndex() -> Success, resolving!');
                    return deferred.resolve(ret);
                })
                .fail(function(error) {
                    if (error) {
                        console.log('Activities.kibana.createIndex() -> ERROR!');
    ...
    activities.kibana.createVisualizationsAndDashboard = function(config, activity, visualizations, user, esClient) {
        var deferred = Q.defer();

        console.log('Activities.kibana.createVisualizationsAndDashboard() -> Started');
        ...
        var Completed = function() {
            ...
            console.log('Activities.kibana.createVisualizationsAndDashboard() -> Vis created ' + numPan + '/' + visualizations.length);
            if (numPan > visualizations.length) {
                var dashboard = activities.kibana.generateDashboard(activity, panels, uiStates);
                activities.kibana.createDashboard(config, activity, dashboard, user, esClient)
                .then(function(res) {

                    console.log('Activities.kibana.createVisualizationsAndDashboard() -> Success, resolving!');
                    deferred.resolve(res);
                })
                .fail(function(err) {
                    console.log('Activities.kibana.createVisualizationsAndDashboard() -> ERROR on creating the dashboard!');
        ...
        if (visualizations.length > 0) {
            for (var i = 0; i < visualizations.length; i++) {
                ...
                var createVis = function(position) {
                    activities.kibana.createVisualization(config, activity, visualizations[position], position, esClient)
                    .then(function(panel) {
                        panels.push(panel);
                        uiStates['P-' + position] = {vis: {legendOpen: false}};

                        console.log('Activities.kibana.createVisualizationsAndDashboard() -> Completing');
                        Completed();
                    })
                    .fail(function(error) {
                        console.log('Activities.kibana.createVisualizationsAndDashboard() -> ERROR on creating visualization!');
                        console.log(JSON.stringify(error, null, 2));
                        return deferred.reject(error);
                    });
                };
    ...
    activities.kibana.createVisualization = function(config, activity, visualizationId, position, esClient) {
        var deferred = Q.defer();
        console.log('Activities.kibana.createVisualization() -> Started!');

        kibana.cloneVisualizationForActivity(activity.gameId, visualizationId, activity._id, activity.rootId, esClient)
            .then(function (result) {
                console.log('Activities.kibana.createVisualization() -> Success, resolving!');
                deferred.resolve('{\"id\":\"' + visualizationId + '_' + activity._id +
                    '\",\"type\":\"visualization\",\"panelIndex\":' + position + ',' +
                    '\"size_x\":6,\"size_y\":4,\"col\":' + (1 + (position - 1 % 2)) + ',\"row\":' +
                    (position + 1 / 2) + '}');
            })
            .fail(function(err) {
                console.log('Activities.kibana.createVisualization() -> ERROR on creating visualization!');
                deferred.reject(err);
            });
    ...
    activities.kibana.createDashboard = function(config, activity, dashboard, user, esClient) {
        var deferred = Q.defer();
        console.log('Activities.kibana.createDashboard() -> started!');

        kibana.createDashboard(dashboard, activity._id.toString(), esClient, config, user)
            .then(function (result) {
                console.log('Activities.kibana.createDashboard() -> success!');
                deferred.resolve(result);
            })
            .fail(function(err) {
                console.log('Activities.kibana.createDashboard() -> ERROR!');
                console.log(JSON.stringify(err, null, 2));
                deferred.reject(err);
            });
