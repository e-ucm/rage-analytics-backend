...
module.exports = (function () {
    ...
    /**
     * Used as a key in the 'extensions' attribute of an object
     * in a statement, to store the moment the session 1 has started
     */
    var firstSessionStartedKey = 'firstSessionStarted';

    /**
     * Used as a key in the 'extensions' attribute of an object
     * in a statement, to store the moment the current session has started.
     * Note that if the current session equals 1 then the 'firstSessionStartedKey' and
     * 'currentSessionStartedKey' are equal.
     */
    var currentSessionStartedKey = 'currentSessionStarted';

    var entries = {};

    var buildEntries = function (gameplayId, versionId, session, firstSessionStarted, currentSessionStarted) {
        entries[gameplayIdKey] = gameplayId;
        entries[versionIdKey] = versionId;
        entries[sessionKey] = session;
        entries[firstSessionStartedKey] = firstSessionStarted;
        entries[currentSessionStartedKey] = currentSessionStarted;
        return entries;
    };
     ...
     * @returns {*}
     */
    var processTraces = function (playerId, versionId, gameplayId, session, firstSessionStarted, currentSessionStarted, data, start) {
        if (!data || !data.length) {
    ...
    return {
         ...
         */
        add: function (authToken, data) {

            return processTraces(authToken.playerId, authToken.versionId, authToken.gameplayId,
                authToken.session, authToken.firstSessionStarted, authToken.currentSessionStarted, data, 0).then(function (convertedData) {
                var promises = [];
                for (var i = 0; i < consumers.length; i++) {
                    promises.push(consumers[i].addTraces(authToken.playerId, authToken.versionId, authToken.gameplayId, data, convertedData));
                }
