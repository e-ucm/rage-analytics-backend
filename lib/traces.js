...
module.exports = (function () {
    ...
    /*
    * Used as a key in the 'extensions' attribute of an object
    * in a statement, to store they ID of a given activity.
    */
    ...
    var buildEntries = function (gameplayId, versionId, activityId, session, firstSessionStarted, currentSessionStarted) {
        ...
        entries[activityIdKey] = activityId;
    ...
    var processTraces = function (playerId, versionId, gameplayId, activityId, session, firstSessionStarted, currentSessionStarted, data, start) {
        ...
        while (i < data.length) {
            ...
            addEntriesToStatement(statement, buildEntries(gameplayId, versionId, activityId, session,
    ...
    return {
         ...
         */
        add: function (authToken, data, activity) {

            return processTraces(authToken.playerId, authToken.versionId, authToken.gameplayId, authToken.activityId,
                authToken.session, authToken.firstSessionStarted, authToken.currentSessionStarted, data, 0).then(function (convertedData) {
                var promises = [];
                for (var i = 0; i < consumers.length; i++) {
                    promises.push(consumers[i].addTraces(authToken.playerId, authToken.versionId, authToken.gameplayId, activity, data, convertedData));
                }
