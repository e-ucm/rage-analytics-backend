...
module.exports = (function () {
    ...
    var wrongFormatError = function (statement) {
        throw {
            status: 400,
            message: 'Statement ' + statement + ' doesn\'t have a valid format.'
        };
    };
    ...
    var processTraces = function (playerId, versionId, gameplayId, data, start) {
        ...
        while (i < data.length) {
            ...
            addEntriesToStatement(statement, buildEntries(gameplayId, versionId));

            var trace = tracesConverter(statement);
            if (!trace) {
                return Q.fcall(wrongFormatError(statement));
            }

            convertedData.push(trace);
            i++;
        }
    ...
    return {
        ...
        add: function (playerId, versionId, gameplayId, data) {
            ...
            return processTraces(playerId, versionId, gameplayId, data, 0).then(function (convertedData) {
                var promises = [];
