...
module.exports = (function () {
    ...
    authTokens.start = function (player, version, activity) {
        var gameplays = null;
        if (activity) {
            gameplays = new Collection(require('./db'), 'gameplays_' + activity._id);
        } else {
            gameplays = new Collection(require('./db'), 'gameplays_' + version._id);
        }
        ...
        return gameplays.find({
            ...
            .then(function (gameplay) {
                ...
                return authTokens.insert(authToken).then(function (authToken) {
                    // First we create a new attempt
                    gameplays.findAndModify(gameplay._id, {
                        $push: {
                            attempts: {
                                number: gameplay.sessions,
                                authToken: authToken.authToken,
                                start: currentSessionStarted,
                                end: null
                            }
                        }
                    }).then(function(gameplay) {
        ...
        return authTokens.track(authorization, true)
            .then(function (authToken) {
                ...
                if (authToken.activityId) {
                    gameplays = new Collection(require('./db'), 'gameplays_' + authToken.activityId);
                } else {
                    gameplays = new Collection(require('./db'), 'gameplays_' + authToken.versionId);
                }
                return gameplays.findAndModify({
                    'attempts.authToken': authorization
                }, {
                    'attempts.$.end': new Date()
                });
            });
    };
     ...
     * @param playerName
     */
    authTokens.getActivityAttempts = function (activityId, playerName) {
        var gameplays = new Collection(require('./db'), 'gameplays_' + activityId);
        if (playerName) {
            return gameplays.find({ playerName: playerName });
        }
        return gameplays.find();
    };
