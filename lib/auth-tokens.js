...
module.exports = (function () {
    ...
    var authTokenSchema = {
        ...
        required: ['authToken','name','gameplayId','versionId','session','playerId','lastAccessed','firstSessionStarted','currentSessionStarted'],
    ...
    authTokens.start = function (player, version, activity) {
        ...
        return gameplays.find(id, player)
            .then(function (gameplay) {
                ...
                gameplays.startAttempt(id, player, authorization).then(function (attempt) {
    ...
    authTokens.track = function (authorization) {
        ...
        if (validationObj.errors && validationObj.errors.length > 0) {
            ...
            return authTokens.findAndModify({
                authToken: authorization
            }, set).then(function (authToken) {
                if (authToken) {
                    return authToken;
                }
