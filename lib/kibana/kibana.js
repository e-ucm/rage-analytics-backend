...
module.exports = (function () {
    ...
    /**
     * Get all teachers of a class. If the class use groupings, return the participants of the grouping, if use groups
     * return the participants of the groups. Else, return the participants of the parent class.
     * @param activityObj
     */
    var getClassTeachers = function (classObj, callback) {
        var deferred = Q.defer();


        var participants = {students: [], teachers: [], assistants: []};
        if (classObj.groupings.length > 0) {
            console.log('using groupings');
            groupings.find({classId: classObj._id}).then(function (groupingsArray) {
                groups.find({classId: classObj._id}).then(function (groupsArray) {
                    classObj.groupings.forEach(function (groupingId) {
                        addParticipantsFromGroupingId(participants, groupingsArray, groupsArray, groupingId);
                        deferred.resolve(participants);
                    });
                }).fail(function (error) {
        ...
        } else if (classObj.groups.length > 0) {
            console.log('using groups');
            groups.find({classId: classObj._id}).then(function (groupsArray) {
                classObj.groups.forEach(function (groupId) {
                    addParticipantsFromGroupId(participants, groupsArray, groupId);
                    deferred.resolve(participants);
                }).fail(function (error) {
                    deferred.reject(error);
                ...
                kibana.getIndexTemplate(indexTemplateId, esClient)
                    .then(function (indexTemplate) {
                        kibana.createIndexWithTemplate(indexName, indexTemplate, esClient)
                            .then(function () {
                                return kibana.createIndexWithTemplate('thomaskilmann-' + indexName, indexTemplate, esClient);
                            })
                            .then(function () {
                                return kibana.buildKibanaResources(indexName, config, esClient);
                            })
    ...
    kibana.createRequiredIndexesForClass = function (classId, user, config, esClient) {
        ...
        esClient.indices.exists({index: classId}, function (err, exists) {
            ...
            if (err || !exists) {
                kibana.getIndexObjectForClass(classId, esClient)
                    .then(function (indexObject) {
                        return kibana.insertIndexObject(classId, indexObject, esClient);
                    })
                    .then(function () {
                        return kibana.createIndexWithTemplate(classId, classIndexTemplate, esClient);
                    })
                    .then(function () {
                        deferred.resolve();
                    })
                    ...
                    if (!error) {
                        ...
                        activities.findById(activityId).then(function (activityObj) {
                            if (activityObj) {
                                ...
                                versions.findById(activityId).then(function (versionObj) {
                                    if (versionObj) {
                                        var users = [user];
                                        kibana.auth(config, function (error, token) {
                                            if (error) {
                                        ...
                                        deferred.reject({message: 'No object found for given id!'});
                                    }
                                });
                            }
                        });
    ...
    kibana.createDashboardToClass = function (dashboard, classId, esClient, config, user) {
        var deferred = Q.defer();
        console.log('kibana.createDashboardToClass() -> started!');

        kibana.addIndexObjectToClass(classId, esClient)
            .then(function () {
                esClient.index({
                    index: '.kibana',
                    type: 'dashboard',
                    ...
                    if (!error) {
                        console.log('kibana.createDashboardToClass() -> ADDED!');

                        var visualizations = JSON.parse(dashboard.panelsJSON);
                        var kibanaResources = ['dashboard_' + classId];
                        ...
                        classes.findById(classId).then(function (classObj) {
                            if (classObj) {
                                getClassTeachers(classObj)
                                    .then(function (participants) {
                                        var users = participants.teachers;
                                        kibana.auth(config, function (error, token) {
                                            if (error) {
                                                return deferred.reject(error);
                                            }
            ...
            .fail(function (error) {
                console.log('Error adding Index Object To Class');
                console.error(error);
                deferred.reject(error);
            });
        ...
        esClient.indices.exists({index: activityId}, function (err, exists) {
            ...
            if (err || !exists) {
                kibana.getIndexObject(activityId, esClient)
                    .then(function (indexObject) {
                        return kibana.insertIndexObject(activityId, indexObject, esClient);
                    })
                    .then(function () {
                        deferred.resolve();
                    })
    ...
    kibana.addIndexObjectToClass = function (classId, esClient) {
        var deferred = Q.defer();

        esClient.indices.exists({index: classId}, function (err, exists) {
            console.log('Including object in new classId');
            if (err || !exists) {
                kibana.getIndexObjectFromClass(classId, esClient)
                    .then(function (indexObject) {
                        return kibana.insertIndexObject(classId, indexObject, esClient);
                    })
                    .then(function () {
