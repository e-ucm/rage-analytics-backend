...
module.exports = (function () {
    ...
    kibana.generateDashboard = function(object, panels, uiStates, extra, type){
        console.log('kibana.generateDashboard() -> Started');
        let deferred = Q.defer();

        let dashboard = '';

        kibana.getDashboardTemplate(object.versionId)
            .then(function(dtobject){
                console.log('kibana.generateDashboard() -> Dashboardtemplate obtained');
                if(dtobject){
                    let role = '';
                    switch (type){
                        case 'class':
                    ...
                    console.log('kibana.generateDashboard() -> Role selected: ' + role);

                    if(dtobject[role]){
                        console.log('kibana.generateDashboard() -> Generating from template');
                        console.log(dtobject[role]);
                        deferred.resolve(kibana.generateDashboardFromTemplate(object._id, dtobject[role]));
                    }else{
                        console.log('kibana.generateDashboard() -> Generating from panels');
                        deferred.resolve(kibana.generateDashboardFromPanels(object._id, panels, uiStates, extra));
                    }
                }else{
                    deferred.resolve(kibana.generateDashboardFromPanels(object._id, panels, uiStates, extra));
                }
    ...
    kibana.generateDashboardFromPanels = function(id, panels, uiStates, extra) {
        console.log('Kibana -> generateDashboardFromPanels');
        ...
        console.log('Kibana -> generateDashboardFromPanels: finished');
        return {
            title: 'dashboard_' + id,
            hits: 0,
            description: '',
    ...
    kibana.generateDashboardFromTemplate = function(activityId, dashboard){
        console.log('kibana.generateDashboardFromTemplate() -> Started');
        dashboard = JSON.stringify(dashboard);

        let match = /dashboard_(.*?)\"/.exec(dashboard);

        if(match.length < 2){
            return false;
        }

        console.log('kibana.generateDashboardFromTemplate() -> Finished');
        return JSON.parse(dashboard.replace(new RegExp(match[1], 'g'), activityId));
    };
            ...
            if (!error) {
                if (response.hits.hits[0] && response.hits.hits[0]._source.kibanaSavedObjectMeta) {
                    ...
                    esClient.index({
                        index: '.kibana',
                        type: 'visualization',
                        id: visualizationId + '_' + activityId,
                        body: obj
            ...
            if (!error) {
                if (response.hits.hits[0] && response.hits.hits[0]._source.kibanaSavedObjectMeta) {
                    ...
                    esClient.index({
                        index: '.kibana',
                        type: 'visualization',
                        id: visualizationId + '_' + versionId,
                        body: obj
                    }, function (error, result) {
                        if (!error) {
                            deferred.resolve(result);
    ...
    kibana.saveDashboardTemplate = function (dashboard, game) {
        console.log('Kibana.saveDashboardTemplate() -> Started!');
        var deferred = Q.defer();

        console.log('Kibana.getDashboardTemplate() -> Finding ' + game);
        dashboardtemplates.findById(game)
            .then(function (dtobject) {
                console.log('Kibana.saveDashboardTemplate() -> finished finding');
                let found = true;
                if (!dtobject) {
                    console.log('Kibana.saveDashboardTemplate() -> not found, creating new!');
                    found = false;
                    dtobject = {
                        _id: dashboardtemplates.toObjectID(game),
                        teacher: null,
                        developer: null
                    };
                };
                ...
                if(found){
                    console.log('Kibana.saveDashboardTemplate() -> Updating');
                    dashboardtemplates.findAndUpdate(game, dtobject)
                        .then(function(updated){
                            console.log('Kibana.saveDashboardTemplate() -> updated!');
                            deferred.resolve(updated);
                        })
                        .fail(function(error){
                            console.log('Kibana.saveDashboardTemplate() -> ERROR');
                            deferred.reject(error);
                        });
                }else{
                    console.log('Kibana.saveDashboardTemplate() -> Inserting!');
                    dashboardtemplates.insert(dtobject)
                        .then(function(inserted){
                            console.log('Kibana.saveDashboardTemplate() -> inserted!');
    ...
    kibana.getDashboardTemplate = function (game) {
        console.log('Kibana.getDashboardTemplate() -> Started!');
        var deferred = Q.defer();

        console.log('Kibana.getDashboardTemplate() -> Finding ' + game);
        dashboardtemplates.findById(game)
            .then(function (dtobject) {
                 console.log('Kibana.getDashboardTemplate() -> finished finding');
                if(!dtobject){
                    console.log('Kibana.getDashboardTemplate() -> not found, creating new');
                    dtobject = {
                        _id: dashboardtemplates.toObjectID(game),
                        teacher: null,
                        developer: null
                ...
                console.log('Kibana.getDashboardTemplate() -> Finished');
                deferred.resolve(dtobject);
            })
            .fail(function (error) {
                console.log('Kibana.getDashboardTemplate() -> ERROR');
                deferred.reject(error);
            });
