...
module.exports = (function () {
                ...
                if (response.hits.hits[0] && response.hits.hits[0]._source.kibanaSavedObjectMeta) {
                    ...
                    if(searchSource.query && searchSource.query.query_string){
                        if(!searchSource.query.query_string.query || searchSource.query.query_string.query === ''){
                            ...
                            searchSource.query.query_string.query = 
                                "(" + searchSource.query.query_string.query + ")"
                                ...
                                getActivityParticipants(activityObj)
                                    .then(function(participants) {
                                        ...
                                        kibana.auth(config, function(error, token) {
                                            ...
                                            console.log('kibana.createDashboard() -> Preparing to update permissions');
                                            kibana.updateKibanaPermissionUserList(config, users, kibanaResources, function(err) {
                                                if (err) {
                                                    return deferred.reject(err);
                                                }
                                                console.log('kibana.createDashboard() -> COMPLETED!');
                                                deferred.resolve();
                                            });
                                        });
                                    })
    ...
    kibana.updateKibanaPermission = function (config, user, resources, callback) {
        console.log('kibana.updateKibanaPermission -> STARTED!');
        var updateKP = function() {
            ...
            request({
                ...
                console.log('kibana.updateKibanaPermission -> Completed in: ', httpResponse.elapsedTime);
    ...
    kibana.updateKibanaPermissionUserList = function (config, users, resources, callback) {
        console.log('kibana.updateKibanaPermissionUserList -> STARTED!');
        var updateKP = function() {
            var baseUsersAPI = config.a2.a2ApiPath;
            request({
                uri: baseUsersAPI + 'applications/look/kibana',
                ...
                console.log('kibana.updateKibanaPermissionUserList -> Completed in: ', httpResponse.elapsedTime);
    ...
    kibana.auth = function(config, callback) {
        if (!rootAuthToken || Math.round(new Date().getTime() / 1000) - lastauth > 300) {
            console.log('Refreshing root auth token');
            kibana.forceAuth(config, callback);
        }else {
            callback(null);
        }
    };

    kibana.forceAuth = function(config, callback) {
        ...
        function (err, httpResponse, body) {
            ...
            lastauth = Math.round(new Date().getTime() / 1000);
