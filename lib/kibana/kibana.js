            ...
            .then(function() {
                esClient.index({
                    ...
                    if (!error) {
                        ...
                        activities.findById(request.params.activityId).then(function (activityObj) {
                            if (activityObj) {
                                getActivityParticipants(activityObj, function(participants) {
                                    ...
                                    async.every(users, function (user, cb) {
                                        kibana.updateKibanaPermission(request.app.config, user, kibanaResources, cb);
                                    }, function (err, result) {
                                        deferred.resolve();
                                    });
                                });
            ...
            .then(function (activityObj) {
                ...
                if (activityObj) {
                    getActivityParticipants(activityObj, function (participants) {
                        var users = participants.students.concat(participants.teachers.concat(participants.assistants));
                        async.every(users, function (user, cb) {
                            kibana.updateKibanaPermission(request.app.config, user, kibanaResources, function (err) {
                                cb();
                            });
                        }, function (err, result) {
