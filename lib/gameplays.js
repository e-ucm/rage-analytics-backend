...
module.exports = (function () {
    ...
    var gameplaySchema = {
        ...
        required: ['playerId','playerName','playerType','animalName','sessions'],
    ...
    gameplays.find = function (id, player) {
        ...
        return g.find({ playerId: player._id.toString()}, true).then(function (gameplay) {
                if (!gameplay) {
                    gameplay = {
    ...
    gameplays.getAttempts = function (id, playerName) {
        var g = new Collection(require('./db'), 'gameplays_' + id);
        if (playerName) {
            return g.find({ playerName: playerName }, true);
        }
        return g.find();
    };
