...
var defaultJarFileName = 'analysis.jar';
var defaultFluxFileName = 'flux.yml';
...
var storm = function (stormConfig, mongodbUrl, zookeeperUrl) {
    ...
    return {
        startTopology: function (sessionId, versionId) {
            ...
            // Check if the files in the /analysis folder exist
            // If they exist, use them (expected: realtime.jar and flux.yaml)

            var analysisDir = stormConfig.analysisFolder + '/' + versionId + '/';

            // Try to read the uploaded flux.yml file
            Fs.readFile(analysisDir + defaultFluxFileName, fsOptions, function (err, src) {

                if (err) {
                    console.log('Failed to read ' + analysisDir + defaultFluxFileName + ' template.' +
                    ...
                    // If the Uploaded flux.yml file could be read, try to start the topology
                    genYMLAndLaunch(src, configPath, analysisDir + defaultJarFileName, config, function (err, output) {
                        if (err) {
